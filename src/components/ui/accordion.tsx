...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn("bg-card rounded-lg overflow-hidden border mb-4 shadow-[0px_24.56px_32.74px_-14.73px_rgba(149,149,149,0.25)]", className)}
    {...props}
  />
))
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "group flex w-full flex-1 items-center px-4 py-6 font-medium text-foreground transition-colors hover:bg-accent/50",
        className
      )}
      {...props}
    >
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Content
    ...
    {...props}
  >
    <div className={cn("py-4 px-4 pl-14 pt-0", className)}>{children}</div>
  </AccordionPrimitive.Content>
))
